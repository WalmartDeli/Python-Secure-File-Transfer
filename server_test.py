...
def keyExchange(connection):
    ...
    shared_key_ser_byte = shared_key_ser.to_bytes(byte_length, 'little')  # Convert server session key to bytes
    connection.send(shared_key_ser_byte)  # Share server shared session key with client
    shared_key_cli_byte = connection.recv(16)  # Receive client shared session key in bytes
    shared_key_cli = int.from_bytes(shared_key_cli_byte, 'little')  # Convert client shared key in int
    if (shared_key_cli == shared_key_ser):  # Compare server and client shared key and if they match proceed
        print("Key match!")
    else:
        exit()
